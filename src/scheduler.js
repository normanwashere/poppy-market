...
export async function initializeScheduler() {
    ...
    const { profile } = state; // Access state.profile directly
    ...
    calendarInstance = new FullCalendar.Calendar(calendarEl, {
        ...
        dateClick: function (info) {
            if (state.profile.role !== 'seller') { // Access state.profile directly
            ...
            const eventsOnDay = calendarInstance.getEvents().filter(event => {
                return event.start.toDateString() === info.date.toDateString() && event.extendedProps.owner_id === state.profile.id && event.extendedProps.status !== 'ended'; // Access state.profile.id
            });
            ...
            eventTitleInput.value = state.profile.full_name; // Access state.profile.full_name
        ...
        eventClick: async (info) => {
            ...
            const isMine = props.owner_id === state.profile.id; // Access state.profile.id
            ...
            if (state.profile.role === 'admin') { // Access state.profile.role
            ...
            } else if (state.profile.role === 'seller') { // Access state.profile.role
            ...
            lucide.createIcons(); // Re-create icons within the modal after innerHTML changes
        }
        ...
        if (bookingForm && !bookingForm._hasBookingFormListener) {
            ...
            bookingForm._hasBookingFormListener = handleBookingSubmit; // Store reference
        }
        ...
        // Channels are managed in main.js showPage, no need to unsubscribe here.
